...
function loadState() {
  ...
  try {
    ...
    if (!state.aiModel) state.aiModel = 'gpt-5.4-nano';
  } catch {
    state.points = structuredClone(samplePoints);
  }
}
...
function syncInputsFromState() {
  ...
  els.aiModel.value = state.aiModel || 'gpt-5.4-nano';
}
...
function syncStateFromInputs() {
  ...
  state.aiModel = els.aiModel.value.trim() || 'gpt-5.4-nano';
}
...
async function callAiEnhancer(point) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      model: state.aiModel || 'gpt-5.4-nano',
      point
    })
  });
...
function bindEvents() {
  [els.subject, els.heading, els.greeting, els.intro, els.closing, els.aiEndpoint, els.aiModel].forEach(input => {
